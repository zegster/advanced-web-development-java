...
public class ListMathServlet extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try 
		{
			...
			//Convert from Object to boolean type
			try 
			{
				isSearch = (boolean)request.getAttribute("isSearch");
				isExact = (boolean)request.getAttribute("isExact");
			}
			catch (Exception e) 
			{
				isSearch = false;
			...
			else
			{
				if(isSearch)
				{
					if(isExact)
					{
						...
						{
							problemlist = problemdao.getProblemListByExactKeyword(keywordsearch);
						}
					}
					else
					{
						...
						{
							problemlist = problemdao.getProblemListByKeyword(keywordsearch);
						}
					}
				}
				else
